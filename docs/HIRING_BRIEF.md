...
| 합성 벤치 (n=32 + Workers AI spot) | https://roomy.page/workbench/evals |
| 소스 · CI | https://github.com/zmzmvkvk/release-workbench |
...
- SSE 스트리밍 · 취소 · 재연결 · 중복 실행 차단
- 도구 인자 수정 / 수정 없이 계속 (HTTP KV, isolate-safe)
- 근거(citation) 없으면 승인 차단
- 실패를 평가 데이터셋에 기록 (`eval.case_recorded`)
- 관찰성: TTFT · cancel latency · reconnect · trace JSON 다운로드

## 하지 않는 주장

- 회사 생산성 % / 사내 화면 / 실사용자 수치
- Workers AI token·cost (provider 미제공 → null, 추정 안 함)
